// LDRX (MIT LICENSE) Antoine LANDRIEUX <https://github.com/AntoineLandrieux/LDRX/>

class LDRX extends HTMLElement{SYMBOL_NULL="Null";SYMBOL_NOTDEF="Undefined";TOKEN_EOF=0xa00;TOKEN_NAME=0xa01;TOKEN_TYPE=0xa02;TOKEN_OPEN=0xa03;TOKEN_CLOSE=0xa04;TOKEN_COMMA=0xa05;TOKEN_STRING=0xa06;TOKEN_NUMBER=0xa07;TOKEN_KEYWORD=0xa08;TOKEN_OPERATOR=0xa09;TOKEN_SEMICOLON=0xa0a;NODE_ROOT=0xb00;NODE_BODY=0xb01;NODE_TYPE=0xb02;NODE_CALL=0xb03;NODE_IF=0xb04;NODE_GET=0xb05;NODE_WHILE=0xb06;NODE_STORE=0xb07;NODE_ARGVAR=0xb08;NODE_STRING=0xb09;NODE_NUMBER=0xb0a;NODE_RETURN=0xb0b;NODE_OUTPUT=0xb0c;NODE_PROMPT=0xb0d;NODE_REMOVE=0xb0e;NODE_FUNCTION=0xb0f;NODE_OPERATOR=0xb10;NODE_PASS=0xb11;ERROR_INTERPRETER=0x01;ERROR_CHARACTER=0x02;ERROR_SYNTAX=0x03;ASSIGNED_ERROR=["Interpreter Error","Unexpected character","Invalid syntax"];ASSIGNED_KEYWORD={"if":this.NODE_IF,"rem":this.NODE_REMOVE,"ask":this.NODE_PROMPT,"while":this.NODE_WHILE,"print":this.NODE_OUTPUT,"return":this.NODE_RETURN};MEMORY=[{name:"LDRX",value:"LDRX (MIT) Antoine LANDRIEUX v1.0.0",arg:null,access:null},{name:"DATE",value:new Date().toDateString(),arg:null,access:null},{name:"TIME",value:Number(new Date()),arg:null,access:null}];constructor(){super();}
...
TYPEs=["string","integer","float","void","any","$"];KEYWORDs=["fn","return","print","while","if","else","rem","ask"];OPERATORs=["<=",">=","==","!=","&&","||","**","*","/","%","^","+","-","&","|","<",">"];Tokenizer(_RawCode){if(!_RawCode)
...
ttype=this.TOKEN_COMMA;else if(_RawCode[index]=="$")
...
let arg=instance_.AST("arg",instance_.NODE_ARGVAR);CurrentToken++;while(1){let argv=ParseMath();if(!argv)
return null;arg.Push(argv);if(_Tokens[CurrentToken].type!=instance_.TOKEN_COMMA)
break;CurrentToken++;}
if(_Tokens[CurrentToken].type!=instance_.TOKEN_CLOSE)
return null;fn.Push(arg);CurrentToken++;return fn;}
...
break;else if(_Tokens[CurrentToken].type==this.TOKEN_SEMICOLON){CurrentAST.Push(this.AST("pass",this.NODE_PASS));CurrentToken++;continue;}
...
return this.ThrowLang(this.ERROR_SYNTAX,_Tokens[CurrentToken].value,_Tokens[CurrentToken].char);CurrentToken++;let arg=this.AST("arg",this.NODE_ARGVAR);while(true){if(_Tokens[CurrentToken].type!=this.TOKEN_TYPE)
...
return this.ThrowLang(this.ERROR_SYNTAX,_Tokens[CurrentToken].value,_Tokens[CurrentToken].char);arg.Push(this.AST(_Tokens[CurrentToken].value,this.NODE_STORE).Push(arg_type));CurrentToken++;if(_Tokens[CurrentToken].type!=this.TOKEN_COMMA)
break;CurrentToken++;}
if(_Tokens[CurrentToken].type!=this.TOKEN_CLOSE)
return this.ThrowLang(this.ERROR_SYNTAX,_Tokens[CurrentToken].value,_Tokens[CurrentToken].char);fn.Push(arg);CurrentAST.Push(fn);CurrentToken++;break;case"{":if(CurrentAST._Children.length==0)
...
return this.ThrowLang(this.ERROR_SYNTAX,keyword,_Tokens[CurrentToken].char);CurrentAST.Push(fn);break;default:return this.ThrowLang(this.ERROR_SYNTAX,_Tokens[CurrentToken].value,_Tokens[CurrentToken].char);}}
return root;}
...
format(_Type,_Value){switch(_Type){case"integer":return parseInt(_Value)||0;case"float":return parseFloat(_Value)||0;case"string":return _Value.toString();case"$":case"any":return _Value;default:return this.SYMBOL_NULL;}}#mem_get(name,access){for(let i=0;i<this.MEMORY.length+1;i++)
...
current=current._Parent._Children[c];tmp=null;c++;}
return null;}
